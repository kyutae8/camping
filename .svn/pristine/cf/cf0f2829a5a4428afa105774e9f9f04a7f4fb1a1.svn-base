...
<form name="joinForm" id="join" method="post" action="<%=request.getContextPath() %>/index.jsp?workgroup=member&work=join_action">
...
<div id="layout_body" class="layout_body">
	...
	<div class="mypage_greeting Pb30">
		<span class="pilsu_icon"><img src="<%=request.getContextPath()%>/images/icon_check1.png"></span>
		<span>항목은<span class="pointcolor">필수 입력</span> 항목입니다.</span>
	</div>
			...
			<div id="formMemberArea" class="resp_member_join">
				<div class="resp_join_table">
					<ul class="required">
						...
						<li class="td"><input type="text" name="id" id="id"	placeholder="공백 없는 영문/숫자 포함 6~20자"><span id="idCheck">아이디 중복 검사</span>
							<div id="idMsg" class="error">아이디를 입력해 주세요.</div>
							<div id="idRegMsg" class="error">아이디는 영문자로 시작되는 영문자,숫자,_의 6~20범위의 문자로만 작성 가능합니다.</div>
							<div id="idCheckMsg" class="error">아이디는 중복 검사를 반드시 실행해 주세요.</div>
						</li>
					</ul>
					<ul class="required">
					...
					<ul>
						...
						<li class="td"><input type="text" name="birth" id="birth" placeholder="ex) 19900512" size="8" maxlength="8">
							<div id="birthMsg" class="error">생일을 입력해 입력해 주세요.</div>
							<div id="birthRegMsg" class="error">생일은 8자리의 숫자로만 입력해 주세요.</div>
						</li>
					</ul>
					<ul class="required">
...
$("#idCheck").click(function() {
	...
	window.open("<%=request.getContextPath()%>/member/id_check.jsp?id=" +$("#id").val(), "idcheck","width=450,height=100,left=700,top=450");
});
