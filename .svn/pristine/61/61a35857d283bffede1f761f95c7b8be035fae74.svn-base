...
<div class="title_container">
	<h2><span>약관 동의</span></h2>
</div>

<div class="resp_login_wrap Mt0">
	<form name="agreeFrm" id="agreeFrm" method="post" action="index.jsp?workgroup=member&work=join">
	<input type="hidden" name="join_type" value="0" id="join_type">
		<div class="mem_agree_area">
			<label id="pilsuAgreeAll" class="pilsu_agree_all"><input type="checkbox" name="agreeCheck" id="agreeCheck"> <span class="pointcolor4" >필수</span> <span>약관 전체 동의</span></label>
			<ul id="agreeList" class="agree_list3">
				<li class="agree_section">
					<a class="agree_view" href="index.jsp?workgroup=company&work=agreement" onclick="showCenterLayer('#agreementDetailLayer')"><span>보기</span></a>
					<label><input type="checkbox" name="agree" value="Y" class="pilsu" > <span>쇼핑몰 이용약관</span> <span class="desc pointcolor4 imp"  >(필수)</span></label>
				</li>
				<li class="agree_section">
					<a class="agree_view" href="index.jsp?workgroup=company&work=agreement" onclick="showCenterLayer('#privacyDeatilLayer')"><span >보기</span></a>
					<label><input type="checkbox" name="agree2" value="Y" class="pilsu"> <span >개인정보 처리방침</span> <span class="desc pointcolor4 imp">(필수)</span></label>
				</li>
			</ul>
		</div>

		<div class="btn_area_c" >
			<button type="submit" id="btn_submit" class="btn_resp size_c color2 Wmax"><span ></span>다음 단계</button>
		</div>
	</form>
</div>

<div id="agreementDetailLayer" class="resp_layer_pop hide">
...
$(document).ready(function() {
	...
	$("#agreeCheck").change(function() {
		//$(selector).is(status) : 검색 엘리먼트(입력태그)의 상태를 비교하여 false/true 를 반환하는 메소드
		// => checkbox, radio, select 기능의 입력태그에서만 사용 가능
		if($(this).is(":checked")) { //체크박스를 선택한 경우
	...
	$("#agreeFrm").submit(function() {
		if($("#join_type").val()=="0") { // 약관에 동의하지 않은 경우
			alert("약관 내용을 읽고 동의해 주세요.")
			return false;
		}
	});
