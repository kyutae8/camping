...
<div id="layout_header" class="layout_header">
		...
		<div class="resp_wrap">
			...
			<ul class="util_wrap_menu2">
				<li class="respCartArea">
					<a href="index.jsp?workgroup=sproduct&work=sproduct_cart"><span>TestCart</span></a>
				</li>
				<li class="respCartArea">
					<a href="index.jsp?workgroup=sproduct&work=myorder"><span>TestOrder</span></a>
				</li>
				<li class="respCartArea">
					<a href="index.jsp?workgroup=cart&work=cart"><span>Cart</span></a>
				</li>
			...
			<% } else {//로그인 사용자 %>
			<% List<ScartDTO> scart=ScartDAO.getDAO().selectAllScart(loginMember.getId());
				String oid=loginMember.getId();
				List<SorderDTO> sorder=SorderDAO.getDAO().selectAllSorderList(oid);
				List<SaleCartDTO> cart = SaleCartDAO.getDAO().selectAllSale(loginMember.getId());
				List<ItemBuyDTO> order = ItemBuyDAO.getDAO().selectId(loginMember.getId());
			%>
			<ul class="util_wrap_menu2">
				<li class="respCartArea">
					<a href="index.jsp?workgroup=sproduct&work=sproduct_cart"><span>TestCart</span><span  class="cart_cnt1" ><%= scart.size() %></span></a>
				</li>
				<li class="respCartArea"> 
					<a href="index.jsp?workgroup=sproduct&work=myorder"><span>TestOrder</span><span class="cart_cnt1"><%= sorder.size() %></span></a>
				</li>
				<li class="respCartArea">
					<a href="index.jsp?workgroup=cart&work=cart"><span>Cart</span><span class="cart_cnt1"><%=cart.size() %></span></a>
				</li>
				<li><a href="index.jsp?workgroup=order&work=order_list" ><span>Order</span><span class="cart_cnt1"><%=order.size() %></span></a></li>
	...
	<div class="logo_wrap">
		<div class="resp_wrap">
			...
			<!-- logo -->
			<h1 class="logo_area">
			<a href="index.jsp?workgroup=base&work=print" target="_self"><img src="<%=request.getContextPath() %>/images/logo.png" alt="(주) 귀사의 회사명" /></a>
			</h1>
