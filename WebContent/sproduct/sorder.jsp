...
for(int i=0; i<scart_num.length; i++){
   int num = Integer.parseInt(scart_num[i]);
   ScartDTO scartDTO = ScartDAO.getDAO().ssl(num); 
   list.add(scartDTO);
}
   
      
for(int w=0; w<list.size(); w++){
    ScartDTO scartDTO = list.get(w);
}   
...
<form name="joinForm" id="join" method="post" action="<%=request.getContextPath() %>/index.jsp?workgroup=sproduct&work=sorder_action">
...
<div id="orderPaymentLayout">
	...
	<div>
		<!-- 주문상품 :: START -->
		<h2 class="title_od1 Pt15"><span>주문상품 정보</spasn></h2>
							...
							<div class="cgd_contents">
								<div class="block block1">
									<ul>
										...
										<select name="qty" class="su" id="su"  >
								...
								<ul class="block block2 x1" id="mobile_cart_sale_tr_44">
									<li class="price_a">
										<span class="ptitle" id="price">상품금액</span><span id="don" class="don"><%=scart.getPrice() %></span>
									</li>
...
<!--  
<input type="hidden" name="color" value="<%=scart.getColor() %>">
-->
<input type="hidden" name="name" value="<%=scart.getName() %>">
<input type="hidden" name="price" value="<%=scart.getPrice() %>">
<input type="hidden" name="num" value="<%=scart.getNum() %>">
<input type="hidden" name="scart_num" value="<%=scart.getScart_num() %>">
			</div>
		</div>
				...
				<div class="resp_join_table">
					...
					<ul class="th ">
						...
						<li class="td">
							...
							<input type="text" id="address1" name="address1" placeholder="주소" readonly="readonly"  value="<%=loginMember.getAddress1() %>">
							<input type="text" id="address2" name="address2" placeholder="상세주소" value="<%=loginMember.getAddress2() %>">
							<input type="text" id="address3" name="address3" placeholder="참고항목" value="<%=loginMember.getAddress3() %>">
					...
					<div class="right_flying_wrap3">
						...
						<div class="order_price_total">
							...
							<ul>
								...
								<li class="td"><span id="sum" class="sum"></span>₩</li>
							</ul>
							<ul>
								<li class="th">
									<span>배송비</span>&nbsp;
...
function calc() {
	...
	*/
	...
	var total = 0;
	$(".cart_goods_detail").each(function(){
		
		var don	= $(this).find(".don").text();
		var su 	= $(this).find(".su").val();
// 		$(".sum").text(numberWithCommas(don*su));
		total += don * su;
	});
	
	$(".sum").text(numberWithCommas(total));
	$("#total").text(total);
}
