			...
			<div class="subpage_container">
				...
				<div id="bbslist">
					...
					<select name="search">
					 ...
					 -->
					<!-- 검색 -->
					<form  action="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=board_list" method="post" name="boardsearch" id="boardsearch">
							...
							<ul class="tbody">
								...
								<li class="subject">
									<ul class="board_goods_list">
										<li class="info">
											<div class="title">  
												<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=board_detail&num=<%=board.getNum()%>&pageNum=<%=pageNum%>&search=<%=search%>&keyword=<%=keyword%>&category=<%=category%>"><!-- 여기에 url 입력 -->
					...
					<% if(startPage>blockSize) { %>
						<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=board_list&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">&lt;&lt;</a> 
						<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=board_list&pageNum=<%=startPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>">이전</a>
					<% } else { %>
					<a class="red">&lt;&lt;</a>
					<a class="red">이전</a>
					<% } %>
					...
					<% for(int i=startPage;i<=endPage;i++) { %>
						<% if(pageNum!=i) { %>
							<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=board_list&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>"><%=i %></a>
						<% } else { %>
							<a class="on red"><%=i %></a>
						<% } %>
					<% } %> 
					...
					<% if(endPage!=totalPage) { %>
						<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=board_list&pageNum=<%=startPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">다음</a>
						<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=board_list&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">&gt;&gt;</a>
					<% } else { %>
					<a class="red">다음</a>
					<a class="red">&gt;&gt;</a>
					<% } %>
					...
					<ul class="bbs_bottom_wrap">
							<li class="right">
								<button type="button" name="boad_write_btn" id="boad_write_btn" class="btn_resp size_b color2" onclick="location.href='<%=request.getContextPath()%>/index.jsp?workgroup=board&work=board_write&category=1'">
									공지사항 쓰기
								</button>
							</li>
...
<!-- 		<script type="text/javascript" src="/data/skin/responsive_camping_healing_gl/common/cscenter_ui.js"></script>
 -->
