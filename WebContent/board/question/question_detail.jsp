			...
			<div class="subpage_container">
				...
				<div id="bbsview">
					...
					<!-- 	<input type="hidden" name="mode" id="mode" value="">
						...
						<table class="table_row_a Thc">
							...
							<tbody>
								...
								<tr>
									<th><p>답변상태</p></th>
									<td colspan="3">
									<%if(board.getAnswer()==null){ %>미처리<%}else{%>	답변완료<%} %>
									</td>
								</tr>
							</tbody>
						</table>
						<div class="goods_origin_info">
							...
							<div id="goodsview" class="goodsviewbox">
								...
								<div class="info" style="width: 50%;">
									...
									<div class="name">
										<!-- <a href="/goods/view?no=49" target="_blank"> -->
											<!-- <span style="color: #7f7777; font-weight: normal; text-decoration: line-through;">90,000₩</span> →  -->
											<%if(board.getItemnum()<=50){ %>
												<%if(item==null){%>
													<span style="color: #cc0000;">64,000₩</span>
												<%} else{%>
													<span style="color: #cc0000;"><%=item.getItem_price() %></span>
												<%} %>
											<%}else{ %>
												<%if(sproduct==null){%>
													<span style="color: #cc0000;">64,000₩</span>
												<%} else{%>
													<span style="color: #cc0000;"><%=sproduct.getPrice() %></span>
												<%} %>
											<%} %>
										<!-- </a> -->
									</div>
						...
						<img src="<%=request.getContextPath()%>/board/review/image/<%=board.getImg()%>" width="100px">
						<%} %>
...
--%>
						...
						<div class="board_detail_contents">
						<hr style="border-top: 1px solid #ddd; display: block;">
						<p>답변 : </p>
						<%=board.getAnswer() %>
...
<!-- 								<form name="cmtform_4" id="cmtform_4" method="post" action="../board_comment_process" target="commentactionFrame"> -->
					...
					<ul class="board_detail_btns">
						...
						<%if(loginMember!=null){ %>
							...
							<button type="button" id="answerBtn" name="goods_boad_modify_btn" board_seq="4" board_id="goods_qna" class="btn_resp size_b">
								<span>답변달기</span>
							</button>
							<%} %>
							<%if(loginMember.getId().equals(board.getId()) || loginMember.getStatus()==9){ %>
							...
							<button type="button" id="goods_boad_write_btn" board_id="goods_qna" class="btn_resp size_b color2" onclick="location.href='<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_write&category=<%=board.getCategory()%>'">
								상품문의 <span>쓰기</span>
							</button>
					...
					$("#removeBtn").click(function() {
						if(confirm("정말로 게시글을 삭제 하시겠습니까?")) {
							$("#boardForm").attr("action","<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_remove_action");
							$("#boardForm").submit();		
						}
					});
					
					$("#modifyBtn").click(function() {
						$("#boardForm").attr("action","<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_modify");
						$("#boardForm").submit();	
					});
					
					$("#listBtn").click(function() {
						$("#boardForm").attr("action","<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_list");
						$("#boardForm").submit();
					});
					
					$("#answerBtn").click(function() {
						$("#boardForm").attr("action","<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_answer");
						$("#boardForm").submit();
					});
					</script>
				</div>
