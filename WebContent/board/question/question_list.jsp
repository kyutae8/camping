...
<%
	...
	if (category == null) category = "3"; //3=상품문의
			...
			<div class="subpage_container">
				...
				<div id="bbslist">
					...
					<select name="search">
					 ...
					 -->
					<!-- 검색 -->
					<form  action="<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_list" method="post" name="boardsearch" id="boardsearch">
					...
					<div class="res_table">
							<ul class="thead">
								...
								<li style="width: 94px;"><span designelement="text" textindex="4" >답변</span></li>
									...
									<ul class="board_goods_list">
										...
										<li class="info">
											...
											<% if(board.getStatus()==0) {//일반 게시글인 경우%>
												<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_detail&num=<%=board.getNum()%>&pageNum=<%=pageNum%>&search=<%=search%>&keyword=<%=keyword%>&category=<%=category%>"><!-- 여기에 url 입력 -->
											...
											<%} %><%else if(board.getStatus()==1){//비밀 게시글인 경우%>
												...
												<%}else{ %>
												<span class="boad_view_btn" style="margin-left: 20px;"> 
												<span class="type"></span>작성자 또는 관리자만 확인 가능합니다.
												</span>
												<%} %>
											<%} %>
					...
					<% if(startPage>blockSize) { %>
						<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_list&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">&lt;&lt;</a> 
						<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_list&pageNum=<%=startPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>">이전</a>
					<% } else { %>
					<a class="red">&lt;&lt;</a>
					<a class="red">이전</a>
					<% } %>
					...
					<% for(int i=startPage;i<=endPage;i++) { %>
						<% if(pageNum!=i) { %>
							<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_list&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>"><%=i %></a>
						<% } else { %>
							<a class="on red"><%=i %></a>
						<% } %>
					<% } %> 
					...
					<% if(endPage!=totalPage) { %>
						<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_list&pageNum=<%=startPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">다음</a>
						<a class="red" href="<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_list&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">&gt;&gt;</a>
					<% } else { %>
					<a class="red">다음</a>
					<a class="red">&gt;&gt;</a>
					<% } %>
					...
					<ul class="bbs_bottom_wrap">
							<li class="right">
								<button type="button" name="boad_write_btn" id="boad_write_btn" class="btn_resp size_b color2" onclick="location.href='<%=request.getContextPath()%>/index.jsp?workgroup=board/question&work=question_write&category=3'">
									상품문의  쓰기
								</button>
							</li>
					<%} %>
