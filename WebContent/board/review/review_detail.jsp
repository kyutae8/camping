					...
					<form name="form1" id="form1" method="post" action="../board_process" target="actionFrame">
						...
						<table class="table_row_a Thc">
							...
							<tbody>
								...
								<tr>
									<th><p>평점</p></th>
									<td colspan="3" style="color: #ff6600"><%=board.getGrade() %></td>
								</tr>
							</tbody>
						</table>
...
<!-- 								<form name="cmtform_4" id="cmtform_4" method="post" action="../board_comment_process" target="commentactionFrame"> -->
					...
					<ul class="board_detail_btns">
						...
						<%if(loginMember!=null){ %>
							...
							<%} %>
							<button type="button" id="goods_boad_write_btn" board_id="goods_qna" class="btn_resp size_b color2" onclick="location.href='<%=request.getContextPath()%>/index.jsp?workgroup=board/review&work=review_write&category=<%=board.getCategory()%>'">
								상품후기 <span>쓰기</span>
							</button>
						<%} %>
					...
					$("#removeBtn").click(function() {
						if(confirm("정말로 게시글을 삭제 하시겠습니까?")) {
							$("#boardForm").attr("action","<%=request.getContextPath()%>/index.jsp?workgroup=board/review&work=review_remove_action");
							$("#boardForm").submit();		
						}
					});
					
					$("#modifyBtn").click(function() {
						$("#boardForm").attr("action","<%=request.getContextPath()%>/index.jsp?workgroup=board/review&work=review_modify");
						$("#boardForm").submit();	
					});
					
					$("#listBtn").click(function() {
						$("#boardForm").attr("action","<%=request.getContextPath()%>/index.jsp?workgroup=board/review&work=review_list");
						$("#boardForm").submit();
					});
					</script>
				</div>
